...
public class ModuleService {
    ...
    public Page<ModuleResponse> list(Pageable pageable){
        ...
        return modules.findByUserId(userId,pageable).map(mapper::toResponse);
    }
    ...
    public ModuleResponse create(UUID projectId, CreateModuleRequest req) {
        ...
        Module saved = modules.save(Module.builder()
                .project(project).name(req.name()).description(req.description()).build());
    ...
    private Module findOwned(UUID id){
        return modules.findByIdAndUserId(id, SecurityUtils.currentUserId())
